    ...
    public class FoodServiceDb : IFoodService
    {
       ...
       // ==================== User Management ==================
        ...
        public User GetUser(int id)
        {
            return db.Users
                     .Include(r => r.Recipes)
                    //  .Include(s => s.RecipeIngredients)
                     .FirstOrDefault(r => r.UserId == id);
        }

        // Add a new user checking email is unique
        ...
        public Recipe CreateRecipe(int userId, string name, Diet diet, MealType mealType, string method, int prepTime, int cookTime, string cuisine, string region, string translator, int calories, int servings, string photoUrl)
        {
        ...
        public IList<Recipe> SearchRecipes(recipeSearch range, string query) 
        {
            ...
            var results = db.Recipes
                            .Include(t => t.User)
                            .Where(t => (t.User.Name.ToLower().Contains(query)
                                        ) &&
